...
public class ShortTruthTableImporter extends PuzzleImporter
{
    ...
    public void initializeBoard(Node node) throws InvalidFileFormatException
    {
        try
        {
            ...
            {
                throw new InvalidFileFormatException("ShortTruthTable Importer: cannot find board element");
            }
            Element boardElement = (Element)node;
            if(boardElement.getElementsByTagName("cells").getLength() == 0)
            {
                throw new InvalidFileFormatException("ShortTruthTable Importer: no data found for board");
            }
...
//                shortTruthTableBoard = new ShortTruthTableBoard(size);
//            }
//            else
//            {
//                throw new InvalidFileFormatException("ShortTruthTable Importer: invalid board dimensions");
//            }
            ...
            for(int y = 0; y < height; y++)
            {
                for(int x = 0; x < width; x++)
                {
                    ...
                    {
                        ShortTruthTableCell cell = new ShortTruthTableCell(0, new Point(x, y));
